...
import { Mic, Loader2, Clock, Video, MessageSquare, Headphones, X, Settings, Users, PhoneOff, Monitor } from "lucide-react";
...
//     
//     // If the host is localhost or 127.0.0.1, connect to the specific WS port 8081.
//     // Otherwise, assume the WS is proxied on the same port as the HTTP server (no explicit port needed).
...
// --- (A) TYPE DEFINITIONS (REQUIRED FOR REACT COMPONENT COMPLETENESS) ---
interface ConversationMessage {
    id: number | string;
    sender: "bot" | "user";
    text: string;
...
interface WSMessage {
    type: "tts_audio" | "stt_started" | "interim_transcript" | "final_transcript" | "session_ended" | "error" | "stt_error" | "warning";
    message?: string;
}

interface TTSResponse extends WSMessage {
    audioBase64: string;
    text: string;
}

interface STTResponse extends WSMessage {
    transcript: string;
}

interface SessionEnded extends WSMessage {
    message: string;
}
...
const MessageBubble = memo(({ msg }: { msg: ConversationMessage }) => (
  <div
    className={`flex w-full ${msg.sender === "user" ? "justify-end" : "justify-start"} mb-3`}
  >
    <div
...
const getOrCreateSession = async (interviewId: string)=> {
  const storedSessionString = sessionStorage.getItem("ai_interview_session");
  console.log("storedSessionString", storedSessionString);
  
  if (storedSessionString) {
    try {
      const storedSession = JSON.parse(storedSessionString);
      const { sessionId, expiresAt } = storedSession;
      
      // Check if session has expired locally (30 minutes = 1800000ms)
      const now = Date.now();
      if (expiresAt && expiresAt < now) {
        console.warn("⏰ Session expired locally. Creating new session.");
        sessionStorage.removeItem("ai_interview_session");
      } else {
        // Validate with server
        const validationResponse = await fetch(`${API_BASE_URL}/validate/session/${sessionId}`);
        if (validationResponse.ok) {
          const data = await validationResponse.json();
          console.log(`✅ Validated existing session: ${sessionId}`);
          const expiryTimestamp = new Date(data.expiresAt).getTime();
          
          // Update sessionStorage with server's expiry time
          ...
          return { sessionId: sessionId, expiresAt: expiryTimestamp };
        }

        console.warn(`❌ Existing session validation failed (Status: ${validationResponse.status}). Creating new one.`);
        sessionStorage.removeItem("ai_interview_session");
      }
    } catch (error) {
      console.error("Error during session validation or parsing. Creating new session.", error);
      sessionStorage.removeItem("ai_interview_session");
    }
  }

  console.log("Creating a new session...");
  const creationResponse = await fetch(`${API_BASE_URL}/create/session`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ interviewId: interviewId || "default-interview-id" }),
  });

  if (!creationResponse.ok) {
    throw new Error(`Failed to create new session on server. Status: ${creationResponse.status}`);
  }
...
// Main component
const InterviewBot: React.FC<{ id: string }> = ({ id }) => {
  // --- (D) REFS and STATES ---
  ...
  const isWaitingForBotResponse = useRef<boolean>(false);
  ...
  // --- (E) INITIALIZATION useEffect ---
  useEffect(() => {
    const initializeSession = async () => {
      setErrorMessage(null);
      try {
        const sessionData = await getOrCreateSession(id);
        setSessionId(sessionData.sessionId);
        setExpiresAt(sessionData.expiresAt);
        console.log(`Session initialized with ID: ${sessionData.sessionId}`);
      } catch (error: any) {
        console.error("Session initialization failed:", error);
        setErrorMessage(`Session setup failed: ${error.message}`);
        setSessionId(null);
        setExpiresAt(null);
      }
    };
  ...
  const convertFloat32ToInt16 = (buffer: Float32Array) => {
    ...
    const buf = new Int16Array(l);
    for (let i = 0; i < l; i++) {
      const sample = Math.max(-1, Math.min(1, buffer[i] * 2));
      buf[i] = sample * 0x7FFF;
    }
    return buf.buffer;
  };

  const sendTTSRequest = useCallback((prompt: string) => {
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
      addMessage("bot", "Connection not ready. Please wait for connection or refresh.", false);
      return;
    }
    if (!sessionId || !expiresAt) {
      addMessage("bot", "Session is not initialized. Cannot send request.", false);
      return;
    }
  ...
  }, [sessionId, expiresAt, id, addMessage]);

  const sendAudioData = useCallback(
    (audioData: ArrayBuffer) => {
      // Fast exit checks for performance
      if (!isRecordingRef.current || wsRef.current?.readyState !== WebSocket.OPEN || !isSTTStreamReadyRef.current) {
        return;
      }
      // 🔇 Don't send audio chunks while bot is speaking to prevent feedback
      if (isBotSpeakingRef.current) {
        return; // Removed console.log for performance
      }
      // 🔇 Don't send audio chunks while waiting for bot response
      if (isWaitingForBotResponse.current) {
        return; // Stop mic during bot API processing
      }
      if (audioData.byteLength === 0) {
        return;
      }
  ...
  const processAggregatedTranscript = useCallback(() => {
    ...
    if (finalText.length > 0) {
      lastSubmittedText.current = finalText;
      isWaitingForBotResponse.current = true;
      addMessage("user", finalText, true);
      sendTTSRequest(finalText);
    }
  }, [addMessage, sendTTSRequest]);

  // Submit answers to the backend and fetch results
  const submitAnswers = async () => {
    if (!sessionId) {
      setErrorMessage("No session ID available. Cannot submit answers.");
      return null;
    }

    try {
      const response = await axios.post(
        `${API_BASE_URL}/interviews/submit-answers?sessionId=${sessionId}`,
        {}
      );
      return response.data;
    } catch (error: any) {
      console.error("Error submitting answers:", error);
      setErrorMessage(`Failed to submit answers: ${error.message}`);
      return null;
    }
  };
  ...
  const startWebSocket = useCallback(() => {
    if (!sessionId) {
      console.log("Waiting for Session ID before starting WebSocket...");
      return;
    }
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      return;
    ...
    if (reconnectAttempts.current >= MAX_RECONNECT_ATTEMPTS) {
      setErrorMessage("Maximum WebSocket reconnection attempts reached. Please refresh.");
      return;
    }

    setWsStatus("connecting");
    const ws = new WebSocket(`${getWsUrl()}/?sessionId=${sessionId}`);
    wsRef.current = ws;
      ...
      try {
        ...
        switch (data.type) {
          case "tts_audio": {
            ...
            // Reset waiting flag when bot response is received
            isWaitingForBotResponse.current = false;
          ...
          case "stt_started":
            ...
            break;
          case "interim_transcript":
            // If user is speaking while waiting for bot response, cancel it
            if (isWaitingForBotResponse.current) {
              console.log("🔄 User speaking again - preparing to cancel bot request");
              setIsLoading(false);
            }
            setLiveTranscript((data as STTResponse).transcript);
            break;
          case "final_transcript": {
            ...
            // Check if user continued speaking while waiting for bot response
            if (isWaitingForBotResponse.current) {
              console.log("🔄 User continued speaking - cancelling previous bot request and appending speech");
              
              // Cancel the loading state
              setIsLoading(false);
              
              // Remove the last user message from conversation (we'll re-add with complete text)
              setConversation(prev => {
                const filtered = prev.slice(0, -1);
                return filtered;
              });
              
              // Prepend the previously submitted text to buffer
              finalTranscriptBuffer.current = [lastSubmittedText.current, transcript.trim()];
              
              // Reset the waiting flag
              isWaitingForBotResponse.current = false;
              lastSubmittedText.current = "";
            } else {
              // Normal flow - just add to buffer
              finalTranscriptBuffer.current.push(transcript.trim());
            }

            setLiveTranscript(finalTranscriptBuffer.current.join(" ") + "...");
            ...
            finalTranscriptTimeout.current = setTimeout(() => {
              processAggregatedTranscript();
            }, 5000); // Wait 5 seconds of silence before submitting
            break;
          }
          case "session_ended":
            endSession();
          ...
          case "stt_error":
          case "warning":
            setErrorMessage(data.message || "A server warning/error occurred.");
            setIsSTTStreamReady(false);
            isSTTStreamReadyRef.current = false;
            isWaitingForBotResponse.current = false;
            lastSubmittedText.current = "";
    ...
    ws.onclose = (event) => {
      ...
      // Only navigate to reports if this was a graceful close (code 1000) or session ended
      const isGracefulClose = event.code === 1000 || event.reason === "Session ended";
      
      if (isGracefulClose && sessionStarted) {
        console.log("✅ Session ended gracefully, navigating to reports...");
        // Clear session from sessionStorage on graceful close
        sessionStorage.removeItem("ai_interview_session");
        router.push(`/viewreports/${sessionId}`);
        return;
      }
      
      // Otherwise attempt to reconnect
      reconnectAttempts.current += 1;
      console.log(`⚡ WebSocket disconnected. Attempt ${reconnectAttempts.current}/${MAX_RECONNECT_ATTEMPTS}. Retrying in 3 seconds...`);
      
      if (reconnectAttempts.current < MAX_RECONNECT_ATTEMPTS) {
        setTimeout(startWebSocket, 3000); // Faster reconnect
      } else {
        setErrorMessage("Connection lost. Please check your internet and refresh.");
      }
    };
  ...
  }, [sessionId, addMessage, processAggregatedTranscript, router, sessionStarted]);

  // --- (H) MEDIA/RECORDING LOGIC ---
  const cleanup = () => {
    if (audioStreamRef.current) {
      audioStreamRef.current.getTracks().forEach((track) => track.stop());
      audioStreamRef.current = null;
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      console.error("Error accessing microphone:", error.message);
      addMessage("bot", `Could not start microphone: ${error.message}. Check permissions.`, false);
      cleanup();
      setIsRecording(false);
      setIsLoading(false);
      setIsSTTStreamReady(false);
  ...
  const handleStartInterview = async () => {
    if (wsStatus !== "connected" || sessionStarted || !sessionId) return;
    ...
    try {
      ...
      console.error("Error starting session:", error);
      addMessage("bot", "Could not start session recording or access media. Continuing without recording.", false);
      setSessionStarted(true);
      sendTTSRequest("Welcome to the interview. Please start speaking.");
    }
  };
  ...
  const endSession = async () => {
    ...
    // Clear session from sessionStorage when interview ends
    console.log("🗑️ Clearing session from sessionStorage");
    sessionStorage.removeItem("ai_interview_session");
    ...
    //   // Optionally redirect to a results page or update state to display results
    //   // For now, log results; you can integrate with a results component
    // }
  };
  ...
  const renderStatusPill = (status: typeof wsStatus) => {
    ...
    if (!sessionId && !errorMessage) {
        return (
            <div className="flex items-center justify-center h-screen">
                <Loader2 size={32} className="w-8 h-8 animate-spin text-blue-600" />
                <p className="ml-3 text-lg text-gray-700">Initializing session...</p>
            </div>
        );
    }
        ...
        return (
            <div className="flex items-center justify-center h-screen bg-red-50">
                <div className="text-center p-8 bg-white shadow-xl rounded-xl">
                    <p className="text-xl font-bold text-red-600 mb-4">Initialization Error</p>
                    ...
                    {sessionStarted && (
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50 max-h-[75vh]">
                                ...
                                {isLoading && (
                                    <div className="flex justify-start">
                                        <div className="bg-gray-200 p-3 rounded-2xl flex items-center space-x-2">
                                            <Loader2 size={16} className="animate-spin text-blue-600" />
                                            <span className="text-xs text-gray-600 animate-pulse">Bot is thinking...</span>
                                        </div>
                                    </div>
                                )}
                                {isRecording && liveTranscript && (
                                    <div className="flex justify-end">
                                        <div className="bg-blue-400 text-white p-3 rounded-2xl opacity-70 max-w-[75%] relative">
                                ...
                                {isRecording && !liveTranscript && (
                                    <div className="flex justify-end">
                                        <div className="bg-blue-100 text-blue-600 px-3 py-2 rounded-2xl text-xs flex items-center space-x-2">
                            ...
                            <button
                                ...
                                title={
                                  isBotSpeaking 
                                    ...
                                    ? "Connecting..." 
                                    : isRecording 
                                    ? "Stop Speaking (or pause for 5 sec)" 
                                    : "Start Speaking"
                                }
                            >
                                <Mic size={20} className={isRecording ? "animate-pulse" : ""} />
                ...
                {!sessionStarted && (
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center z-50">
                        <div className="text-center max-w-md mx-auto px-6">
                            ...
                            <button
                                onClick={handleStartInterview}
                                disabled={wsStatus !== "connected" || !sessionId}
                                className="px-8 py-4 text-lg font-semibold rounded-xl shadow-xl transition-all duration-300 transform bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white active:scale-95 disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed disabled:transform-none"
                            >
                                {wsStatus === "connected" ? "Start Interview" : "Connecting..."}
                            </button>
                            {(wsStatus === "connecting" || !sessionId) && (
                                <div className="mt-4 flex items-center justify-center space-x-2 text-sm text-gray-500">
                                    <Loader2 size={16} className="animate-spin" />
                                    <span>{sessionId ? "Establishing connection..." : "Setting up session..."}</span>
                                </div>
                            )}
                        </div>
                    </div>
